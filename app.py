...
with st.sidebar:
    ...
    st.markdown("### 🔬 Research & Pedagogical Tutor")
    st.caption("• Domain-Aware Source Prioritization\n• Intelligent 'Understand This Topic' Tutor\n• Interactive Speech Synthesis & Multi-Format Exports")
...
st.markdown(
    '<p style="font-size: 1.05rem; opacity: 0.85; margin-bottom: 1.5rem; line-height: 1.5;">Autonomous research engine & intelligent topic tutor. Investigates the web, cross-checks evidence across diverse domains, and teaches concepts with verified citations.</p>',
    unsafe_allow_html=True,
)


# -----------------------------------------------------------------------------
# RESEARCH QUESTION INPUT & MODE CONTROLS
# -----------------------------------------------------------------------------
...
if start_clicked:
    ...
    if not target_question:
        ...
        stages_ui = {
            ...
            "report": "5. Citation Synthesis & Topic Tutor Guide",
        }
...
if result:
    if not result.success:
        ...
        with tab_tutor:
            st.markdown(
                f"<div class='tutor-card'>"
                f"<h2 style='font-size: 1.45rem; font-weight: 700; margin-top: 0;'>🎓 Understand This Topic: {result.question}</h2>"
                f"<p style='opacity: 0.85; margin-bottom: 0;'>Pedagogical topic guide explaining core mechanisms, terminology, and practical real-world impact.</p>"
                f"</div>",
                unsafe_allow_html=True,
            )
            ...
            components.html(
                get_voice_controller_html("en", clean_tutor_text),
                height=140,
            )
        ...
        with tab_report:
            ...
            components.html(
                get_voice_controller_html("en", clean_report_text),
                height=140,
            )
